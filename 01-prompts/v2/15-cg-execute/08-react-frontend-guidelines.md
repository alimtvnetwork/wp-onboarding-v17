...
14. [ ] /learn Ingest `02-spec/02-coding-guidelines/02-typescript/` for domain-specific architectural specifications.
15. [ ] /learn Ingest `02-spec/07-design-system/` for domain-specific architectural specifications.
...
- Context Diet: When spawning a subagent, DO NOT paste file contents, memory logs, or the entire plan into its prompt. Give it the absolute minimal instruction (e.g., "Read subtask file `.ai-memory/plans/subtasks/XX-react-frontend/01-<subtask-title>.md` and execute it"). The subagent MUST read the necessary files itself.
...
- **Where to save it:** Save this master plan into `.ai-memory/plans/pending/XX-react-frontend-audit.md` listing every affected file, exact line numbers, and the extraction plan.
- **Create a Task-Specific Rule Set:** Analyze the specific domain and write 3-5 custom rules inside the spec file.
- **Subtasks:** Break the plan down into granular subtask files inside `.ai-memory/plans/subtasks/XX-react-frontend/` (e.g. `01-decompose-oversized-components.md`, `02-hook-named-objects.md`).
...
- [ ] **`02-spec/02-coding-guidelines/02-typescript/09-typescript-standards-reference.md`**
...
- [ ] **`02-spec/02-coding-guidelines/02-typescript/15-state-management.md`**
...
- [ ] **`02-spec/02-coding-guidelines/02-typescript/10-promise-await-patterns.md`**
...
WHILE (STEP < PHASE_2_STEPS):
    ...
    1. Read the next subtask from .ai-memory/plans/subtasks/XX-react-frontend/
    ...
    7. When all subtasks are finished and local CI is 100% green:
          - Move .ai-memory/plans/pending/XX-react-frontend-audit.md to .ai-memory/plans/completed/
...
- [ ] Master Guidelines: I have fully read and strictly enforced every file in `02-spec/02-coding-guidelines/02-typescript/` and `.ai-memory/coding-guidelines.md`.
