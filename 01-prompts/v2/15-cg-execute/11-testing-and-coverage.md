...
- Context Diet: When spawning a subagent, DO NOT paste file contents, memory logs, or the entire plan into its prompt. Give it the absolute minimal instruction (e.g., "Read subtask file `.ai-memory/plans/subtasks/XX-testing-and-coverage/01-<subtask-title>.md` and execute it"). The subagent MUST read the necessary files itself.
...
- **Where to save it:** Save this master plan into `.ai-memory/plans/pending/XX-testing-and-coverage-audit.md` listing every function, its line count, missing test cases (positive, negative, error), and decomposition plan.
...
- **Subtasks:** Break the plan down into granular subtask files inside `.ai-memory/plans/subtasks/XX-testing-and-coverage/` (e.g. `01-auth-service-tests.md`, `02-order-flow-integration-tests.md`).
...
WHILE (STEP < PHASE_2_STEPS):
    ...
    1. Read the next subtask from .ai-memory/plans/subtasks/XX-testing-and-coverage/
