...
9. Root `readme.md` and `.ai-memory/what-to-read.md` stay in sync. Same file list, same order, no drift. Every write-memory run updates both.
...
14. `.ai-memory/what-to-read.md` (or `.ai-memory/what-to-read.md`)
...
```sh
mv .ai-memory/ambiguous-questions/01-new-ambiguity/01-<slug>.md .ai-memory/ambiguous-questions/02-ambiguity-resolved/01-<slug>.md
```
...
1. Verbatim spec capture and learning persistence:
   - Every sizeable user directive, decision, architectural rule, or spec from the session is saved verbatim under `.ai-memory/memory/01-<slug>.md` or `.ai-memory/memory/learned/01-<slug>.md`.
   - Reference from `.ai-memory/memory/01-index.md`.
   - Reflect in `.ai-memory/plan.md` / `.ai-memory/plans/01-index.md` if it changes the roadmap.
...
2. Consolidation policy:
   ...
   - Detailed / high-value specs: STRICTLY FORBIDDEN TO CONSOLIDATE. Any spec containing detailed requirements, edge cases, domain architecture (`02-spec/21-app/`), error-handling matrices (`02-spec/03-error-manage/`), coding rules (`02-spec/02-coding-guidelines/`), or user instructions must NEVER be merged, summarized, or shortened.

3. New user command / convention: `.ai-memory/02-spec/commands/01-<slug>.md`.

## Phase 6: `.ai-memory/what-to-read.md` and Root `readme.md`
...
Sync root `readme.md` with `.ai-memory/what-to-read.md`. Ensure root `readme.md` is strictly lowercase (auto-fix and commit/push if uppercase).

## Phase 7: Consistency Validation (Self-Test Before Completion)

Before you reply, check every item:

- [ ] `.ai-memory/plans/01-index.md` lists every file in `pending/` and `completed/`?
- [ ] Every file in `completed/` has `Status: completed` in its frontmatter?
- [ ] `.ai-memory/memory/01-index.md` lists every file in `memory/`?
...
11. [ ] Verbatim user directives and recent conversations captured under `.ai-memory/memory/` or `.ai-memory/memory/learned/`.
...
15. [ ] `.ai-memory/what-to-read.md` present, changelog-prepended with UTC ISO 8601 timestamp, list in sync with Pre-flight and `readme.md`.
