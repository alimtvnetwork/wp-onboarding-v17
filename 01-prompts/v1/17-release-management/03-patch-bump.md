...
Trigger Keywords & Aliases: `patch bump`, `patch release`, `bump patch version`, `hotfix bump`, `bugfix release`

Only use this prompt when the user explicitly requests a `patch bump`, `patch release`, or hotfix. Otherwise, any generic release trigger defaults to MINOR per Rule 0.
...
## RULE 0: Patch Version Calculation Standard (Bugfixes & Hotfixes Only)
...
2. **Patch Bump Arithmetic:** `MAJOR.MINOR.PATCH` becomes `MAJOR.MINOR.(PATCH+1)`.
   - `MAJOR` and `MINOR` digits remain identical.
   - Example: `v6.42.3` -> `v6.42.4`.
3. State the previous version and new version explicitly in the reply before touching any file.
4. Execute only when user explicitly designated `patch` or `hotfix`.
...
All patch version bumps and releases MUST strictly follow this 5-step Git release lifecycle:
...
[STEP 2] Bump Version on Release Branch via Python Script:
         python 03-ai-scripts/37-bump-version.py --tier patch --scope "<Bugfix scope>"
         (or python .ai-memory/release/bump_versions.py --type patch)
         Updates version.json, package.json, readme.md, changelog.md, and runs sync.
                   │
                   ▼
[STEP 3] Stage & Commit on Release Branch:
         git commit -m "release: vX.Y.Z <bugfix scope>"
                   │
                   ▼
...
2. **Step 2: Bump Version on Release Branch via Python Script:**
   Execute the dedicated Python bump script (`03-ai-scripts/37-bump-version.py --tier patch` or `.ai-memory/release/bump_versions.py --type patch`) on the release branch.
...
3. **Step 3: Commit in Release Branch:**
   Stage and commit all version bump and generated release files on the release branch: `git commit -m "release: vX.Y.Z <bugfix scope>"`.
...
1. **Changelog:** You MUST read the `"changelog"` configuration from `version.json` (e.g. `file_path` and `format`) and append the proper changelog correctly according to that format.
   - Header format: `## [vX.Y.Z] YYYY-MM-DD <short headline>`
...
- [ ] Determine new version: `MAJOR.MINOR.PATCH` -> `MAJOR.MINOR.(PATCH+1)`.
...
- [ ] Step 2: Bump version on release branch using the repository-aware Python bump script (`python 03-ai-scripts/37-bump-version.py --tier patch` or `python .ai-memory/release/bump_versions.py --type patch`).
...
- [ ] Step 3: Stage and commit all release changes on release branch: `git commit -m "release: vX.Y.Z <bugfix scope>"`.
