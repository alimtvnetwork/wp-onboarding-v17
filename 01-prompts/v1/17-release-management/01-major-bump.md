...
Trigger Keywords & Aliases: `major bump`, `major release`, `bump major version`, `breaking bump`, `breaking release`

Execute this prompt whenever a major version bump or breaking change release is requested.
...
## RULE 0: Major Version Calculation Standard (Breaking Changes)
...
2. **Major Bump Arithmetic:** `MAJOR.MINOR.PATCH` becomes `(MAJOR+1).0.0`.
   - **MANDATORY RESET:** Both `MINOR` and `PATCH` MUST reset strictly to `0`!
   - Example: `v6.42.3` -> `v7.0.0`.
3. State the previous version and new version explicitly in the reply before touching any file.
4. Do NOT ask for confirmation. Do NOT open plan mode. Execute the major bump directly.
...
All major version bumps and releases MUST strictly follow this 5-step Git release lifecycle:
...
[STEP 1] Create & Switch to Dedicated Release Branch:
         git checkout -b release/vX.0.0
         (NEVER bump directly on main or active work branch)
                   │
                   ▼
[STEP 2] Bump Version on Release Branch via Python Script:
         python 03-ai-scripts/37-bump-version.py --tier major --scope "<Breaking change scope>"
         (or python .ai-memory/release/bump_versions.py --type major)
         Updates version.json, package.json, readme.md, changelog.md, and runs sync.
                   │
                   ▼
[STEP 3] Stage & Commit on Release Branch:
         git commit -m "release: vX.0.0 <breaking change scope>"
                   │
                   ▼
...
[STEP 5] Put Commit Back to Main Branch & Push:
         git checkout main
         git merge release/vX.0.0
         git push origin main
         git push origin release/vX.0.0
...
1. **Step 1: Create & Switch to Release Branch:**
   Create and checkout the dedicated release branch first: `git checkout -b release/vX.0.0`. Releases MUST NOT be committed directly to `main` without a release branch.
2. **Step 2: Bump Version on Release Branch via Python Script:**
   Execute the dedicated Python bump script (`03-ai-scripts/37-bump-version.py --tier major` or `.ai-memory/release/bump_versions.py --type major`) on the release branch.
...
3. **Step 3: Commit in Release Branch:**
   Stage and commit all version bump and generated release files on the release branch: `git commit -m "release: vX.0.0 <breaking change scope>"`.
4. **Step 4: Create Annotated Git Tag:**
   Create the annotated tag on that release commit: `git tag -a vX.0.0 -m "Release vX.0.0"`.
5. **Step 5: Put Commit Back to Main Branch & Push:**
   Switch to `main` (`git checkout main`), merge the release branch commit (`git merge release/vX.0.0`), push `main`, `release/vX.0.0`, and tag `vX.0.0` to `origin`, then restore the starting branch if different from `main`.
...
1. **Changelog:** You MUST read the `"changelog"` configuration from `version.json` (e.g. `file_path` and `format`) and append the proper changelog correctly according to that format.
   - Header format: `## [vX.0.0] YYYY-MM-DD <short headline>`
   - Document breaking changes explicitly under `### Breaking Changes` or `### Changed`.
...
### Install <Project Name> vX.0.0

To pin your repository to this exact version, run the following one-liner:
Unix/Bash: `curl -sL https://raw.githubusercontent.com/<owner>/<repo>/vX.0.0/install.sh | bash -s -- ".ai-memory/prompts" "vX.0.0"`
PowerShell: `Invoke-WebRequest -Uri https://raw.githubusercontent.com/<owner>/<repo>/vX.0.0/install.ps1 -OutFile install.ps1; .\install.ps1 -TargetDir ".ai-memory/prompts" -Version "vX.0.0"`
...
1. **Quick Install One-Liners:**
   - Binary repos: `irm https://github.com/<owner>/<repo>/releases/download/vX.0.0/install.ps1 | iex` (PowerShell) and `curl -fsSL https://github.com/<owner>/<repo>/releases/download/vX.0.0/install.sh | bash` (Bash).
   - Script/Prompt repos: `Invoke-WebRequest ...` (PowerShell) and `curl -sL ... | bash -s -- ".ai-memory/prompts" "vX.0.0"` (Bash).
2. **Extracted Changelog:** The exact `[vX.0.0]` section from `changelog.md`.
3. **Pass Notes File:**
   ```bash
   gh release create "vX.0.0" --title "vX.0.0" --notes-file ".ai-memory/release/release-notes-vX.0.0.md" --generate-notes
   ```
...
- [ ] Determine new version: `MAJOR.MINOR.PATCH` -> `(MAJOR+1).0.0` (both MINOR and PATCH reset to 0).
...
- [ ] Step 1: Create and switch to dedicated release branch: `git checkout -b release/vX.0.0`.
- [ ] Step 2: Bump version on release branch using the repository-aware Python bump script (`python 03-ai-scripts/37-bump-version.py --tier major` or `python .ai-memory/release/bump_versions.py --type major`).
...
- [ ] Step 3: Stage and commit all release changes on release branch: `git commit -m "release: vX.0.0 <breaking change scope>"`.
- [ ] Step 4: Create annotated tag on release commit: `git tag -a vX.0.0 -m "Release vX.0.0"`.
- [ ] Step 5: Switch to `main`, merge `release/vX.0.0`, push `main`, `release/vX.0.0`, and tag `vX.0.0` to `origin`, then restore starting branch.
