...
Read the overarching big plan of the main task from `.ai-memory/plans/pending/01-<slug>.md`. You must follow this plan strictly.
...
- The `<slug>` is derived directly from the plan filename. If the plan file is `01-auth-refactor.md`, subtasks live under `.ai-memory/plans/subtasks/01-auth-refactor/01-<subslug>.md`. Never guess or invent a slug — read the filename.
...
- Do not write randomly into `.ai-memory`. Plans live exclusively in `.ai-memory/plans/pending/01-<slug>.md` and subtasks under `.ai-memory/plans/subtasks/01-<slug>/01-<subslug>.md`.
...
The plan file at `.ai-memory/plans/pending/01-<slug>.md` and the subtask files under `.ai-memory/plans/subtasks/01-<slug>/01-<subslug>.md` are the SINGLE source of truth for all coordination between the main agent and sub-agents. Every status update MUST go there. This is how the main agent knows what is running, what is done, and when to proceed.

Every sub-agent that is spawned MUST follow this lifecycle without exception:

- Step 1 — Read: The sub-agent reads its assigned subtask file at `.ai-memory/plans/subtasks/01-<slug>/01-<subslug>.md`. It must understand the full scope, acceptance criteria, and affected files before touching any code. It also checks the parent plan at `.ai-memory/plans/pending/01-<slug>.md` for overall context.
...
- Step 4 — Mark Done & Signal: Once the task is complete, the sub-agent MUST:
  - Update its subtask file at `.ai-memory/plans/subtasks/01-<slug>/01-<subslug>.md` flipping status to `✅ Done`, listing every file it changed, and writing a one-line summary of what was done.
  - Update the corresponding step in the parent plan file `.ai-memory/plans/pending/01-<slug>.md` with `✅ Done` on that step entry.
...
- If a sub-agent stalls, gives garbage, or fails 3 times, rollback its dirty files (`git checkout -- <files>`), log root cause to `.ai-memory/plan.md`, and spawn a new one.
...
Once ALL sub-agents have signaled completion and updated their subtask files in `.ai-memory/plans/subtasks/`:
...
- [ ] Read overarching main task plan from `.ai-memory/plans/pending/01-<slug>.md`.
