...
1. **Single-Agent Unified Blueprint Mandate:**
   - The initial plan, instruction scoping, and lookahead roadmap MUST be authored by a single lead agent first as a unified blueprint before delegating work to subagents.
...
3. **Execution Mode (Disjoint Instruction Generation):**
   - Subagents execute parallel disjoint instruction authoring tasks across non-overlapping domain areas.
